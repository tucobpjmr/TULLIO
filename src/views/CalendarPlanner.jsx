...
import { getAssignableTeam, formatTime, isActiveTask } from "../utils/core.js";
...
const CalendarPlanner = ({ state, dispatch }) => {
  ...
  const agentWeekDays = viewMode === "week" ? weekDays : getWeekDays(0);
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <button onClick={() => viewMode === "month" ? setCurrentMonth(new Date(year, month - 1)) : setWeekOffset(w => w - 1)} style={{
              background: "#fff", border: "1px solid var(--border)", borderRadius: 8, width: 34, height: 34, cursor: "pointer", fontSize: 14
            }}>←</button>
            <button onClick={() => { viewMode === "month" ? setCurrentMonth(new Date()) : setWeekOffset(0); setSelectedDay(null); }} style={{
              background: "var(--gold)", color: "var(--navy)", border: "none", borderRadius: 8, padding: "0 14px", height: 34, cursor: "pointer", fontSize: 12, fontWeight: 700
            }}>Oggi</button>
            <button onClick={() => viewMode === "month" ? setCurrentMonth(new Date(year, month + 1)) : setWeekOffset(w => w + 1)} style={{
              background: "#fff", border: "1px solid var(--border)", borderRadius: 8, width: 34, height: 34, cursor: "pointer", fontSize: 14
            }}>→</button>
            ...
            {weekDays.map((day, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  <div style={{ padding: "10px 10px 6px", background: isToday ? "var(--gold)" : "var(--surface2)", textAlign: "center" }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: isToday ? "var(--navy)" : "var(--text-muted)" }}>{dayNames[i]}</div>
                    <div style={{ fontSize: 20, fontWeight: 700, color: isToday ? "var(--navy)" : "var(--text)" }}>{day.getDate()}</div>
                  </div>
