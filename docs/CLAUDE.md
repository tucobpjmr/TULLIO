...
- **Gli avvisi dell'advisor Supabase si accettano per NOME**: `verifica-advisor` ha un elenco esplicito (`AVVISI_ACCETTATI`) con le nove funzioni `SECURITY DEFINER` esposte di proposito; qualunque altro WARN fa fallire, compreso uno nuovo su una tabella aggiunta domani. Finché "WARN" significava "accettato", `auth_leaked_password_protection` — che nessuno ha mai deciso di accettare — è rimasto invisibile per due audit (ST-14/B-2). ⚠️ Quel rilievo è ancora APERTO e non è correggibile da codice: Supabase → Authentication → Password → *Enable leaked password protection*. Riverificato sull'advisor live il **12 agosto**: ancora `WARN`. È il **terzo audit di fila** che lo riporta (ST-14, B-2 dell'8 agosto, B-3 dell'11) su un'app il cui accesso è a sola password — non ha bisogno di un'altra analisi, ha bisogno di quel click
