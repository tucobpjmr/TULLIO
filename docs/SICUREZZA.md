...
| `sonda_audit_clients_update()` | `private.can_clienti_scrittura()` (admin/manager/agent, attivo e non pending — lo stesso predicato delle policy di `clients`) + `rate_limit_incrementa` a 20/ora per chiamante. M-2 dell'audit del 5 settembre per la sonda, **M-1 del 10 settembre per i due gate**: era senza, e «non ha nulla da proteggere» valeva per il dato restituito, non per il carico — è una definer che scrive `clients` scavalcando la RLS, quindi chiamabile in ciclo da qualunque login. Usata da `.github/workflows/rls.yml` | ok, dal 10 settembre |
