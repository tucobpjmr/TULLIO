...
- **Letture che devono arrivare INTERE**: si paginano con `fetchAllRows` + `WITH_COUNT` da `src/lib/pagination.js`, mai con una `select` nuda. PostgREST tronca ogni select a `db-max-rows` (1000 di default) rispondendo **HTTP 200 senza errore**: le righe oltre la soglia non arrivano e `error` è `null`, quindi il difetto non fallisce da solo — si manifesta come un dato che "non esiste". `Clients.list()` ha convissuto con 818 righe su quel cap (ST-3). Serve anche un ordinamento **deterministico**, cioè chiuso su una colonna unica (`.order('name').order('id')`): senza, due pagine consecutive possono ripetere o saltare una riga. Blindato da `src/test/paginazione.test.js`. Il prossimo candidato è `Tasks.list` (256 righe, cestino incluso), che porta commenti e cronologia annidati: lì `count: 'exact'` ha un costo da misurare prima
