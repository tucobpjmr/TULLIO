...
| [`AUDIT_ARCHITETTURA_2026-08-12.md`](AUDIT_ARCHITETTURA_2026-08-12.md) | Architettura, struttura, separazione delle responsabilità, duplicazione, anti-pattern React e sicurezza — **primo audit che verifica anche lo stato reale della produzione e della CI**, non il solo repository (12 agosto 2026) | **C-1, A-1 e A-3 ✔ chiusi il 12 agosto** (§4), gli altri dieci aperti. **C-1** (critico) era il rilievo che dettava il calendario: `TaskThreads.comments()`/`history()` leggono senza paginazione due tabelle a crescita monotona — `task_history` è a 621 righe con ~14,8/giorno, cioè **~26 giorni** dal cap `db-max-rows`, dopo i quali la cronologia sarebbe sparita dalla UI a ogni evento realtime senza alcun errore. Corretto: `TaskThreads.comments/history` e `Tasks.list` passano ora da `fetchAllRows`, e con loro non resta una sola lettura del data layer che possa essere troncata in silenzio (il costo di `count: 'exact'` sulla select annidata dei task, l'unica riserva che teneva ferma la terza, è stato misurato: 11 ms contro un timeout di 8 s). Blindato da 12 nuovi casi in `paginazione.test.js` — 1150 test. **A-1** ✔ e **A-2** non sono difetti di codice ma di sorveglianza: il workflow *Verifica RPC* era rosso a ogni run dall'8 agosto perché la `20260808120000` risultava non applicata — in realtà lo era (verificata colonna per colonna e funzione per funzione), mancava la sola riga in `schema_migrations`, ora registrata. **A-2 ◐ parzialmente chiuso**: il controllo advisor non ha **mai** girato perché `SUPABASE_ACCESS_TOKEN` non è configurato e lo script usciva 0 in silenzio — corretta la parte di codice (un'annotazione `::warning` rende ora il salto visibile in cima al log e nel riepilogo del run, verificato eseguendo lo script senza token), ma configurare il secret stesso richiede due azioni fuori dalla portata di questa sessione: generare un Personal Access Token dalla dashboard Supabase e salvarlo nelle impostazioni GitHub del repository, entrambe legate all'account di chi amministra il progetto. `get_advisors` riconferma 0 ERROR e i 10 WARN tutti già in `AVVISI_ACCETTATI`: nessuna sorpresa in attesa quando il secret sarà configurato. **A-3** ✔ era un consolidamento lasciato a metà — `views/archiveFilters.js` nato per unificare tre copie di `filterByPeriod`/`PERIOD_OPTIONS`/`chipStyle`, `Trash.jsx` restato con la terza — corretto passando `Trash.jsx` alle stesse funzioni condivise di `Archive.jsx`/`ArchivedListe.jsx`: −32 righe (458 da 490), la stessa etichetta ("Sempre") in tutte e tre le viste. ⟦stato: 3/13 chiusi⟧ |
