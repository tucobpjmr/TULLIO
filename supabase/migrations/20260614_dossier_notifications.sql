-- Caveat #28 — Notifiche pratiche (dossier_status + dossier_departure)
--
-- STATO: applicato via apply_migration MCP il 14/6/2026 (version 20260614212448);
--        questo file e' per version control (stesso pattern di
--        20260614_mention_composite_names.sql). Allinea repo <-> DB dopo
--        l'UI deep-link gia' pronta in PR #56 (handoff v17).
--
-- 1. Trigger su UPDATE OF status -> notifica 'dossier_status' a created_by +
--    manager/admin attivi non-pending (escluso l'attore).
-- 2. pg_cron giornaliero notify_dossier_departure -> pratiche confermate /
--    in_corso con partenza nei prossimi 3 giorni. De-dup 20h.
--
-- Pattern identico alle notifiche task (Step F/J): le notifiche nascono SOLO
-- da trigger/funzioni server-side (RLS vieta l'insert dal client). SECURITY
-- DEFINER per poter inserire bypassando la policy "own notifications".
...
-- nei prossimi 3 giorni. De-dup: salta se gia' notificato nelle ultime 20h.
