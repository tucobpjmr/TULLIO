...
import {
  ...
  formeDuplicate, formeGiaInComune, suffissoDiCollisione, doppioNome, testSciolti,
...
describe('testSciolti', () => {
  it('segnala un test scritto direttamente in src/test/', () => {
    expect(testSciolti(['setup.js', 'nuovoCaso.test.jsx'])).toEqual(['nuovoCaso.test.jsx']);
  });

  it('non segnala i file che NON sono test', () => {
    // `setup.js` sta lì per configurazione (vite.config.js lo nomina), e le
    // cartelle non arrivano fin qui: il chiamante filtra sui soli file.
    expect(testSciolti(['setup.js'])).toEqual([]);
  });

  it('riconosce tutte e quattro le estensioni', () => {
    expect(testSciolti(['a.test.js', 'b.test.jsx', 'c.test.ts', 'd.test.tsx'])).toHaveLength(4);
  });

  it('SOLLEVA se la cartella risulta vuota', () => {
    // «zero test sciolti» e «zero test» sono la stessa cifra e due affermazioni
    // diverse: se la lettura si rompe, questo controllo deve fallire e non
    // passare a vuoto.
    expect(() => testSciolti([])).toThrow(LetturaFallita);
  });
});
