...
export const Notifications = {
  ...
  markRead: async (id) => {
    const supabase = await getSupabase();
    return supabase.from('notifications').update(withOrigin({ read: true })).eq('id', id);
  },
  markAllRead: async () => {
    const supabase = await getSupabase();
    return supabase.from('notifications').update(withOrigin({ read: true })).eq('read', false);
  },
  ...
  markReadForConversation: async (conversationId) => {
    const supabase = await getSupabase();
    return supabase.from('notifications').update(withOrigin({ read: true }))
  ...
  removeRead: async () => {
    const supabase = await getSupabase();
    return supabase.from('notifications').delete().eq('read', true);
  },
  ...
  removeAll: async () => {
    const supabase = await getSupabase();
    return supabase.from('notifications').delete().not('id', 'is', null);
  },
};
...
export const Push = {
  ...
  getVapidPublicKey: async () => {
    const supabase = await getSupabase();
    return supabase.rpc('get_vapid_public_key');
  },
  // Upsert: ri-attivare sul solito dispositivo aggiorna le chiavi senza duplicare.
  save: async (row) => {
    const supabase = await getSupabase();
    return supabase.from('push_subscriptions').upsert(row, { onConflict: 'user_id,endpoint' });
  },
  ...
  findByEndpoint: async (userId, endpoint) => {
    const supabase = await getSupabase();
    return supabase.from('push_subscriptions').select('id')
  ...
  sendTest: async () => {
    const supabase = await getSupabase();
    return supabase.rpc('send_test_push');
  },
};
