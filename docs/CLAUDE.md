...
- **Gli avvisi dell'advisor Supabase si accettano per NOME**: `verifica-advisor` ha un elenco esplicito (`AVVISI_ACCETTATI`) con le nove funzioni `SECURITY DEFINER` esposte di proposito; qualunque altro WARN fa fallire, compreso uno nuovo su una tabella aggiunta domani. Finché "WARN" significava "accettato", `auth_leaked_password_protection` — che nessuno ha mai deciso di accettare — è rimasto invisibile per due audit (ST-14/B-2). ⚠️ Quel rilievo è ancora APERTO e non è correggibile da codice: Supabase → Authentication → Password → *Enable leaked password protection*
